...
describe('models', () => {
  ...
  test('maps ui aliases to provider api model ids', () => {
    ...
    expect(mapUiModelToApi('kimi-2.5')).toBe('kimi-k2.5')
  })
  ...
  test('returns provider for ui and api model values', () => {
    ...
    expect(getProviderForModel('gemini-3-pro-preview')).toBe('gemini')
  ...
  test('returns model specific reasoning options', () => {
    ...
    expect(getReasoningEffortOptions('gemini-3-pro-preview')).toEqual(['low', 'high'])
    expect(getReasoningEffortOptions('kimi-2.5')).toEqual(['enabled', 'disabled'])
  })
  ...
  test('coerces unsupported reasoning values to model defaults', () => {
    ...
    expect(getReasoningEffortForModel('gemini-3-pro-preview', 'medium')).toBe('high')
    expect(getReasoningEffortForModel('kimi-2.5', 'high')).toBe('enabled')
    expect(getReasoningEffortForModel('gpt-5.2', 'x-high')).toBe('xhigh')
  })

  test('applies model feature flags', () => {
    expect(supportsReasoningEffort('kimi-2.5')).toBe(true)
    expect(supportsTemperature('kimi-2.5')).toBe(false)
